...
class DatabaseClient(object):
    ...
    def db(self):
        ...
        url = TIMESERIES_DB.get("URL") or f"http://{TIMESERIES_DB['HOST']}:{TIMESERIES_DB['PORT']}"
        return self.InfluxDBClient(url=url, token=self.token, org=self.org)

    @cached_property
    def _write_api(self):
        """Returns write API instance."""
        from influxdb_client.client.write_api import SYNCHRONOUS #docs https://influxdb-client.readthedocs.io/en/stable/api.html#influxdb_client.WriteApi
    ...
    def create_or_alter_retention_policy(self, name, duration):
        ...
        try:
            ...
            logger.warning(
                f'Bucket "{self.db_name}" not found. Create database first.'
            )
            return
        duration_seconds = self._duration_to_seconds(duration)
        bucket.retention_rules = [BucketRetentionRules(
            type="expire", every_seconds=duration_seconds
        )] #docs https://influxdb-client.readthedocs.io/en/stable/api.html#influxdb_client.domain.Bucket.retention_rules
    ...
    def _duration_to_seconds(self, duration):
        """Converts duration string (eg '30d') to seconds."""
        unit = duration[-1]
        value = int(duration[:-1])
        mapping = {"s": 1, "m": 60, "h": 3600, "d": 86400, "w": 604800}
        return value * mapping.get(unit, 86400)
    ...
    def write(self, name, values, **kwargs):
        ...
        # Log if database or retention_policy are specified (because they are not used in influxdb2, as they are managed at bucket level)
        ...
        if kwargs.get("retention_policy"):
            logger.warning(
                f'Parameter "retention_policy" is managed at bucket level in InfluxDB 2.x'
            )
        point = {
            "measurement": name,
        ...
        try:
            self._write_api.write(
                bucket=self.db_name, org=self.org, record=point #docs https://influxdb-client.readthedocs.io/en/stable/api.html#influxdb_client.WriteApi.write
            )
            ...
            for table in tables:
                for record in table.records:
                    ...
                    for tag_key in record.values.keys():
                        if tag_key not in ["_time", "_measurement", "_field", "_value"]:
                            result[tag_key] = record.values[tag_key]
    ...
    def read(self, key, fields, tags, **kwargs):
        ...
        # Return list of points like v1 (not QueryResultSet)
        return list(self.query(flux_query).get_points())
    ...
    def delete_metric_data(self, key=None, tags=None):
        ...
        try:
            ...
            self._delete_api.delete(
                start, stop, predicate, bucket=self.db_name, org=self.org # docs https://influxdb-client.readthedocs.io/en/stable/api.html#influxdb_client.DeleteApi.delete
            )
    ...
    def get_query(self, chart_type, params, time, group_map, summary=False, 
                fields=None, query=None, timezone=settings.TIME_ZONE):
        ...
        if chart_type == "wifi_clients":
            ...
            flux_query += ' |> sum() |> map(fn: (r) => ({r with _value: r._value / 1000000000}))'
